...
| `search_thoughts` | Hybrid retrieval (vector kNN ∪ trigram, fused by RRF, recency-boosted, optionally reranked by a cross-encoder — see [Reranking](#reranking-search-results)). Gracefully degrades to trigram-only when the embedder is unreachable; result includes `vector_search_available: bool`. Scope filtering: pass `scope` for exact match, or `scope_prefix` to query across a namespace of related scopes (e.g. `scope_prefix: "rjf."` matches `rjf.work`, `rjf.personal`, etc.) — mutually exclusive, supply at most one. Accepts an optional `tag_filter` (JSONB containment against `thoughts.tags` — e.g. `{"kind": "task"}`, `{"entities": ["engram"]}`, or `{"people": ["Sarah"]}`) and surfaces each hit's full `tags` object alongside the content. Excludes retracted thoughts. |
...
| `link_thoughts` | Create a link from a thought to a polymorphic target (another thought, an entity name, a person name, or a URL) with one of seven closed-vocabulary relations (`replaces`, `requires`, `references`, `supports`, `belongs_to`, `decided_by`, `refines`). Idempotent on the `(from, relation, to_kind, to_value)` quadruple. |
...
| `get_related_thoughts` | Walk the link graph from a thought. Returns `outbound` + `inbound` groups; each hit carries a `to_kind`/`to_value` discriminator. For thought-target hits, also surfaces the target thought's content-preview, scope, retraction state, and timestamps. Optional filters by relation, target kind, and direction. |
| `list_scopes` | Enumerate scopes currently in use (each with `thought_count`, `first_activity_at`, `last_activity_at`), sorted most-recently-used first. Optional `prefix` filter. Discover what's in the corpus before capturing; pair with `scope_prefix` on `search_thoughts` / `recent_thoughts` to query across a namespace of related scopes. |

CLI subcommands: `engram serve`, `engram worker`, `engram migrate`, `engram embed-backfill [--scope X | --scope-prefix Y]`, `engram tag [--rerun --since <RFC3339>] [--scope X | --scope-prefix Y]`, `engram audit migrations [--since <RFC3339>] [--limit N]`. Operational details in [`DEVELOPMENT.md`](DEVELOPMENT.md).
...
`relations` (M6.1+) is the tagger's first auto-emission of relational edges. The v5 prompt instructs the model to extract `(relation, to_kind, to_value)` triples for **explicit relational claims** in prose — URLs cited, named entities the thought belongs to, persons it's attributed to. Drainer inserts each emission into `thought_links` with `source = 'tagger'` (distinguishable in `get_related_thoughts` responses via the `link_source` field). v1 ships non-thought targets only (`url` / `entity` / `person`); thought-to-thought tagger relations require entity resolution and are deferred. The schema enforces `maxItems: 5`, closed enums on `relation` and `to_kind`, and a 2048-char cap on `to_value`. On re-tag (`engram tag --rerun`), prior tagger-emitted edges from each thought are soft-deleted before fresh emissions land — agent-supplied edges are unaffected.
...
Edges are agent-supplied via MCP (`link_thoughts(from, relation, {to_thought_id | to_entity | to_person | to_url}, note?)`) — there's no LLM extraction yet. The closed vocab is intentionally small: it captures the relational structure that actually shows up in conversation memory without trying to be a general knowledge graph.

**Polymorphic targets (M5.2).** The `from` side is always a thought; the `to` side can be a thought (UUID), a free-text entity name (e.g. "Probe 2 experiment"), a free-text person name (for attribution), or a URL (must start with `http://` or `https://`). Pass exactly one of `to_thought_id` / `to_entity` / `to_person` / `to_url`. Entities and persons are free-text strings — engram has no first-class entity or person table.

**Idempotency.** `link_thoughts` is idempotent on the `(from, relation, to_kind, to_value)` quadruple — re-asserting the same live edge returns the existing `link_id` with `is_new: false`. Mirrors `capture`'s content-fingerprint dedup.

**Soft-delete (M5.2).** `unlink_thoughts` soft-deletes via `thought_links.deleted_at`. Returns a three-way `status`: `deleted_now` (was live, just removed), `already_deleted` (was previously removed), or `never_existed`. Re-creating a previously-removed edge via `link_thoughts` succeeds — the partial unique index ignores soft-deleted rows.

**Traversal.** `get_related_thoughts(thought_id, relations?, target_kinds?, direction?)` returns grouped `outbound` (edges where this thought is `from`) and `inbound` (edges where it's `to`) arrays. Each hit carries `to_kind`/`to_value`; thought-target hits additionally surface `thought_id`, `scope`, `content_preview` (first 400 chars), retraction state, and timestamps. `target_kinds` filters outbound to a subset of `[thought, entity, person, url]` (no effect on inbound — inbound is always thought→thought by schema). Soft-deleted edges are excluded.
...
**Out of scope.** Tagger-extracted relations (the LLM finding "this refines the earlier finding" in prose and emitting an edge automatically) — that's M5.x; requires entity resolution. Reverse traversal from non-thought targets ("what thoughts link to this URL?") — deferred. Restoring a soft-deleted edge as a tool — operator can re-link via `link_thoughts` (fresh row) or psql-edit `thought_links.deleted_at`.
...
| [M5 — selective relations](docs/milestones/m5-selective-relations.md) | ✅ | Thought-to-thought graph layer: closed relation vocabulary (initially `replaces`, `requires`, `references`, `belongs_to`, `decided_by`, `refines`; M5.1 added `supports` after dogfood), `thought_links` table, three new MCP tools (`link_thoughts`, `unlink_thoughts`, `get_related_thoughts`). M5.2 closed the loop with heterogeneous targets (`to_entity` / `to_person` / `to_url`), soft-delete + three-way unlink status, and operator audit (`migration_audit` + `engram audit migrations`). |
| [M6 — stats CLI + tagger-extracted relations](docs/milestones/m6-stats-and-tagger-relations.md) | ✅ | `engram stats` CLI subcommand prints corpus + storage telemetry without psql. v5 tagger emits non-thought relations (URLs / entities / persons) from prose with `source='tagger'`; M5.2's polymorphic targets + soft-delete handle the data shape. The original M6 (artifacts) was dropped after a live-corpus measurement showed engram occupies a high-signal-density sweet spot; M5.2's `to_url` already covers the "reference external doc" case. |

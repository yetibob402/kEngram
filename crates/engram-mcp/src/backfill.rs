...
mod tests {
    ...
    use engram_core::{EmbeddingModel, Metadata, Scope, Source, ThoughtId};
    ...
    async fn raw_insert(pool: &PgPool, content: &str, scope: &str) -> ThoughtId {
        ...
        let (inserted, _) = engram_storage::insert_thought(
            pool,
    ...
    async fn heals_unembedded_thoughts_then_drains(pool: PgPool) {
        ...
        assert!(
            engram_storage::thought_has_embedding(&pool, id, good.model())
                .await
                .unwrap()
        );
    }
    ...
    async fn respects_scope_filter(pool: PgPool) {
        ...
        assert!(
            engram_storage::thought_has_embedding(&pool, w_a, good.model())
                .await
                .unwrap()
        );
        assert!(
            engram_storage::thought_has_embedding(&pool, w_b, good.model())
                .await
                .unwrap()
        );
        assert!(
            !engram_storage::thought_has_embedding(&pool, p, good.model())
                .await
                .unwrap()
        );
    }

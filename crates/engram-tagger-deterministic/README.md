# engram-tagger-deterministic

Reference sidecar implementation for the engram HTTP-tagger wire
...
[`engram-tagger-protocol`](../engram-tagger-protocol/).

## What this is for

You have two reasons to care about this crate:

1. **Direct use.** Run the sidecar as-is, point engram at it
...
2. **Reference for writing your own.** Want a non-LLM tagger in
   ...
   that speaks engram's wire contract from end to end — read
   `src/main.rs` for the HTTP shape, read
   `../engram-tagger-protocol/src/lib.rs` for the JSON spec, then
   reimplement in your language of choice.

## What this is NOT

A drop-in replacement for engram's default LLM tagger. The 2026-05-24
...
Per [`engram-tagger-protocol`](../engram-tagger-protocol/) (the serde
...
for engram-style content. Copy it somewhere your sidecar can read:

```sh
mkdir -p ~/.config/engram
cp topic-taxonomy.toml ~/.config/engram/topic-taxonomy.toml
```
...
```sh
cargo run --release -p engram-tagger-deterministic
# Or with custom env:
EMBEDDER_ENDPOINT=http://localhost:11434/v1 \
  GLINER_MODEL_DIR=$HOME/models/gliner_small-v2.1 \
  TOPIC_TAXONOMY_PATH=$PWD/crates/engram-tagger-deterministic/topic-taxonomy.toml \
  cargo run --release -p engram-tagger-deterministic
```
...
```sh
docker build -t engram-tagger-deterministic:latest \
  -f crates/engram-tagger-deterministic/Dockerfile .
...
Drop this alongside your existing engram stack:
...
services:
  tagger-deterministic:
    build:
      context: .
      dockerfile: crates/engram-tagger-deterministic/Dockerfile
    environment:
      EMBEDDER_ENDPOINT: http://tei:80  # or your bge-m3 endpoint
      GLINER_MODEL_DIR: /models/gliner
      TOPIC_TAXONOMY_PATH: /etc/engram/topic-taxonomy.toml
    volumes:
...
2026-05-24 calibration values, validated against engram's dogfood
...
- [`../engram-tagger-protocol/`](../engram-tagger-protocol/) — wire spec

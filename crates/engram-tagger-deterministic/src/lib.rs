//! engram-tagger-deterministic — reference sidecar implementation for
//! the engram HTTP-tagger wire contract.
//!
...
//!   the `engram-tagger-protocol` wire shape so engram (or any other
...
//! gline-rs + ort native build cost — this isn't engram's default
...
//! prototypes via the supplied [`engram_core::Embedder`] at startup.
...
//!
//! Calibrated against engram's local Ollama (bge-m3) + a corpus of 25
...
use engram_core::{Embedder, EmbedderError, ScopeVocab, TagOutput, Tagger, TaggerError, Tags};
...
/// `engram_cli::config::DeterministicTaggerConfig` (the figment-parseable
/// shape) — kept separate so this crate doesn't depend on engram-cli for
...
impl Default for DeterministicTaggerConfig {
    fn default() -> Self {
        ...
        Self {
            ...
            // Relative path: resolves to `crates/engram-extract/topic-taxonomy.toml`
            // when `cargo test` runs in this crate, and to `<run-dir>/topic-taxonomy.toml`
            // in production. Operators should set this explicitly in `engram.toml`
            // (e.g. `topic_taxonomy_path = "/etc/engram/topic-taxonomy.toml"`).
            ...
            // fixtures empty at 0.60 (real bge-m3 cosine on engram content
...
impl DeterministicTagger {
    ...
    /// Used by `engram-cli`'s server path, which only needs to stamp
...
mod tests {
    ...
    /// `cargo test -p engram-extract` runs with CWD at the crate root,

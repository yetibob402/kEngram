...
//! beside `engram-extract/Cargo.toml`; configurable per-deployment via
...
use engram_core::{Embedder, TaggerError};
...
mod tests {
    ...
    // Mini "tempfile" replacement — engram doesn't currently take a
    ...
    mod tempfile_like {
        ...
        impl NamedTempFile {
            pub fn new() -> Self {
                ...
                let path =
                    std::env::temp_dir().join(format!("engram-taxonomy-test-{pid}-{n}.toml"));

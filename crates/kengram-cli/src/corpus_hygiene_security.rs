...
ALTER FUNCTION lock_thought_relation_endpoints(uuid[],boolean,uuid[]) OWNER TO kengram_gate_owner;
...
ALTER FUNCTION lock_thought_relation_endpoints(uuid[],boolean,uuid[]) SECURITY DEFINER SET search_path = pg_catalog, public;
...
REVOKE ALL ON FUNCTION lock_thought_relation_endpoints(uuid[],boolean,uuid[]) FROM PUBLIC,kengram_runtime;

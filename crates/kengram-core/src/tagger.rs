//! The `Tagger` trait — the seam between kengram and whatever backend
//! turns a thought's content into a bag of metadata tags. Implementations
//! live in `kengram-extract`.
//!
...
/// the full `LinkTarget` for insertion via `kengram_storage::insert_link`.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
/// - `HttpTagger` — speaks kengram's own JSON wire contract (see
///   `kengram-tagger-protocol`) for sidecar implementations in any
///   language. Reference sidecar lives at
///   `crates/kengram-tagger-deterministic/`.
/// - `FakeTagger` — deterministic in-memory tagger for tests.
///
/// External implementations are welcome and don't require forking kengram;
/// they can either implement this trait directly (in-process, Rust) or
/// run as a sidecar speaking the `kengram-tagger-protocol` wire shape (any
/// language, over HTTP).
#[async_trait]

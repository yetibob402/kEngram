//! `HttpTagger` — kengram's HTTP client for the tagger sidecar pattern.
//!
//! Talks to any sidecar (any language) that implements the
//! `kengram-tagger-protocol` wire shape: `POST {endpoint}/tag` with a
//! [`TagRequest`] body, returns a [`TagResponse`]. The reference sidecar
//! lives at `crates/kengram-tagger-deterministic/`; operators wanting a
//! non-Rust tagger ship their own service in their preferred language
//! and point kengram at it via `provider = "http"` in `[tagger]`.
//!
...
//! is the wire shape: this client speaks kengram's own JSON contract
//! rather than the OpenAI chat-completions shape, so sidecars don't have
//! to fake LLM semantics to plug in.

use async_trait::async_trait;
use kengram_core::{ScopeVocab, TagOutput, Tagger, TaggerError};
use kengram_tagger_protocol::{PROTOCOL_VERSION, ScopeVocabHint, TagRequest, TagResponse};
use reqwest::Client;
...
pub struct HttpTaggerConfig {
    ...
    pub endpoint: String,
    /// Kengram-side stable identity written into
    /// `thoughts.tags_extractor_model`. Conventionally
    /// `"<vendor>/<sidecar>"`. The sidecar's actual model identity isn't
    /// echoed on the wire — kengram stamps what the operator configures
    /// so provenance lives in `kengram.toml`, not in the sidecar's
    /// response.
    pub model_id: String,

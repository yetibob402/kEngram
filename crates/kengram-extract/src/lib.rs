//! kengram-extract: `Tagger` implementations.
//!
...
//! - [`HttpTagger`] talks to a sidecar speaking kengram's own JSON wire
//!   contract (see `kengram-tagger-protocol`). Use for non-LLM taggers
...
//!   mirrors `kengram-embed::FakeEmbedder` in shape.
//!
//! The `Tagger` trait itself lives in `kengram-core` (`kengram_core::Tagger`)
//! so the tag drainer loop in `kengram-mcp` / `kengram-cli` can depend on the

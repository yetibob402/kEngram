...
/// comparable. Operator runs `kengram tag --rerun --since 1970-01-01T00:00:00Z`
...
/// about databases (kengram API design, branding, capture discipline,
...
/// end of the topics paragraph ("a thought naming kengram and
...
/// normalization step in `kengram-mcp::drain`. The post-v9 retag
...
/// validator in `kengram-mcp::validate` — strips any `entities` entry
...
/// against `crates/kengram-extract/tests/fixtures/use_mention.json`
...
/// inspect it (`kengram-cli` can print it; configuration can compare against
...
/// shouldn't be considered comparable; `kengram tag --rerun` then re-tags
...
- entities: default to []. Only emit a name that the thought MENTIONS BY ITS SURFACE NAME — a specific named thing (project, product, library, tool, technology, organization) with its own canonical identity independent of this thought. Examples of valid entities: \"kengram\", \"pgvector\", \"PostgreSQL\", \"MCP\", \"TCGplayer\", \"Cap'n Proto\", \"Rust\", \"Hummingbird\". Preserve the thought's casing, or use canonical casing if the thought is inconsistent.
...
/// excluded — they get post-process normalization in `kengram-mcp::drain`
...
mod tests {
    ...
    async fn valid_response_parses_to_tags() {
        ...
        assert_eq!(
            ...
            vec!["kengram".to_string(), "pgvector".to_string()]
        );
        assert_eq!(
    ...
    async fn valid_response_with_relations_parses_to_tag_output() {
        use kengram_core::{ExtractedTarget, RelationKind};
        let server = MockServer::start().await;
        Mock::given(method("POST"))
            .and(path("/v1/chat/completions"))
            .respond_with(
    ...
    async fn vocab_section_flows_into_request_body() {
        ...
        assert!(sys.contains("kengram"), "entities flow into prompt vocab");
        // v11: topics no longer render into the prompt — they get post-process
        // normalization in kengram-mcp::drain instead. The bundled prompt's
    ...
    /// `cargo test -p kengram-extract --features integration -- live_vllm`.
    ...
    async fn live_vllm_round_trip() {
        ...
        let tags = t
            .tag(
                "Kengram uses pgvector for vector storage. Sarah will review the migration plan.",
                None,
            )
            .await
    ...
    fn diagnostic_fixtures() -> Vec<(
        ...
        vec![
            ...
            (
                ...
                "kengram's MCP tool surface exposes capture, search_thoughts, get_thought, recent_thoughts, and retract_thought. Search supports hybrid vector + trigram retrieval with optional cross-encoder rerank.",
            ),
            (
                "22bccb3a",
                "kengram.test",
    ...
    ///
    /// Run with:
    /// `cargo test -p kengram-extract --features integration --release -- kind_stability_diagnostic --nocapture --ignored`
    ///
    ...
    /// scope vocab>)` matching what the worker tick / `kengram tag --rerun`
    ...
    ///
    /// Run with:
    /// `cargo test -p kengram-extract --features integration --release -- kind_stability_diagnostic_with_vocab --nocapture --ignored`
    #[cfg(feature = "integration")]
    #[tokio::test]
    #[ignore]

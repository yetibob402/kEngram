...
impl KengramServer {
    ...
    #[tool(
        description = "Hybrid search across captured thoughts. Combines vector kNN (over the active embedding model) with bounded trigram lexical similarity via reciprocal rank fusion, then applies a recency boost. The trigram leg is intentionally temporary and opportunistic for the latency cutover: large legacy blobs make pg_trgm non-selective, so the leg is budgeted and can return no hits until Phase-2 FTS replaces it as the proper lexical leg. On natural-language queries the trigram leg's surface-similarity threshold typically gates everything out, so it primarily contributes a co-occurrence boost when a query hits exact surface terms (acronyms, proper nouns, code identifiers). For typical content-search the candidate pool is dominated by vector kNN with rerank as the final discriminator; the trigram leg may return no hits if it exceeds its latency budget. When a cross-encoder reranker is configured, the top `candidate_pool` post-RRF hits are re-scored and returned in rerank order. Scope filtering: use `scope` for exact match or `scope_prefix` for namespace match — supply at most one (mutually exclusive; supplying both returns an error). Optional `tag_filter` narrows to thoughts whose tags JSONB satisfies a containment query. Scope filter (whichever you pick), `tag_filter`, and the search query all compose via AND. If the embedder is unreachable, `vector_search_available` is false and search returns whatever bounded lexical hits are available; if the reranker fails, results come back in RRF + recency order and `rerank_used` is false. Each hit carries the thought's tags so consumers can show / threshold without a follow-up get_thought. For each hit, follow up with `get_related_thoughts(thought_id)` to walk the graph layer — refinements, replacements, supports, citations, and other edges the agent has linked. The search-then-traverse pattern is how a discovery walk arrives at the relational context of a hit."
    )]

...
//! over the `kengram-tagger-protocol` HTTP wire contract.
//!
//! Operators run this binary either standalone or inside a Docker
//! container, then point kengram at it via `provider = "http"` in
...
//!                           (advertised back to kengram; stamped on tag rows)
...
use kengram_embed::{OpenAICompatibleConfig, OpenAICompatibleEmbedder};
use kengram_tagger_deterministic::{DeterministicTagger, DeterministicTaggerConfig};
use kengram_tagger_protocol::{PROTOCOL_VERSION, TagRequest, TagResponse};
...
async fn main() -> anyhow::Result<()> {
    ...
    tracing::info!(
        ...
        "kengram-tagger-deterministic: resolved config",
    );
...
async fn tag_handler(
    ...
    let vocab = req.vocab.map(kengram_core::ScopeVocab::from);
    let out = state
        .tagger
...
/// transient-vs-non-transient classification kengram's drainer relies on.
#[derive(Debug)]
enum ApiError {
    BadRequest(String),
    /// Maps to 5xx — kengram's drainer will treat as transient and retry.
    Transient(String),
    /// Maps to 4xx (non-500 non-2xx) — kengram will treat as non-transient.
    NonTransient(String),
}
...
impl From<kengram_core::TaggerError> for ApiError {
    fn from(e: kengram_core::TaggerError) -> Self {
        if e.is_transient() {

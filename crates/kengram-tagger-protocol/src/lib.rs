//! Wire contract for the kengram tagger sidecar pattern.
//!
//! Kengram's HTTP-tagger client (`kengram_extract::HttpTagger`) and any
...
//!   as **transient** by kengram's drainer — the job stays in the queue
//!   and is retried on the next tick.
//! - **4xx** + malformed response bodies are **non-transient** —
//!   kengram logs and skips the thought without retry.
//!
//! See `kengram_core::TaggerError::is_transient`.
//!
//! # Versioning
//!
...
//! field. Kengram's client refuses to use a sidecar whose declared
//! version doesn't match what the client sends.

use serde::{Deserialize, Serialize};

pub use kengram_core::{ExtractedRelation, ScopeVocab, Tags};
...
/// Request body for `POST {endpoint}/tag`. Sent by kengram's HTTP
...
/// `kengram-core` and is `PartialEq + Clone + Default` but not `Serialize`,
...
/// on a successful tag; parsed by kengram's HTTP tagger client into
/// `kengram_core::TagOutput`.
#[derive(Debug, Clone, Default, PartialEq, Serialize, Deserialize)]
pub struct TagResponse {
    /// The protocol version the sidecar speaks. Optional on the wire
    /// (sidecars may omit it for backward-compat with version-1
    /// clients that don't check). When present, kengram's client
